		...
		<div id="header">
			...
			<form action="<?php echo $__Context->root_url ?>" method="get" class="search"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" />
				...
				<fieldset>
					...
					<input type="image" src="/files/attach/textyle/396/img/buttonSearch.gif" alt="<?php echo $__Context->lang->cmd_search ?>" class="btnSearch" />
				</fieldset>
			</form>
			<hr />
			<div class="extension">
        ...
        <div id="body">
            <div id="content">
                ...
                <?php }else if($__Context->textyle_mode == 'guestbook'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','guestbook.html') ?>
                
                <?php }else if($__Context->textyle_mode=='tags'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','tags.html') ?>
                
                <?php }else if($__Context->textyle_mode=='comment_form'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','comment_form.html') ?>
                 
                <?php }elseif($__Context->textyle_mode=='list'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','post.title_list.html') ?>
                
                <?php }elseif($__Context->textyle_mode=='summary'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','post.summary.html') ?>
                
                <?php }else{ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('files/attach/textyle/396','post.list.html') ?>
                <?php } ?>
