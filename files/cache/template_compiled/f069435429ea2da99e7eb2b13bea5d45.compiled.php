<?php if(!defined("__XE__"))exit;?><div id="xe" class="fixed">
	<div id="container" class="c">
		<div id="header">
			...
			<form action="<?php echo $__Context->root_url ?>" method="get" class="search"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" />
				...
				<fieldset>
					...
					<input type="image" src="/modules/textyle/skins/happyLetter/img/buttonSearch.gif" alt="<?php echo $__Context->lang->cmd_search ?>" class="btnSearch" />
				</fieldset>
			</form>
			<hr />
			<div class="extension">
        ...
        <div id="body">
            <div id="content" class="content">
                ...
                <?php }elseif($__Context->textyle_mode == 'profile'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','profile.html') ?>
                
                <?php }else if($__Context->textyle_mode == 'guestbook'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','guestbook.html') ?>
                
                <?php }else if($__Context->textyle_mode=='tags'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','tags.html') ?>
                
                <?php }else if($__Context->textyle_mode=='comment_form'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','comment_form.html') ?>
                 
                <?php }elseif($__Context->textyle_mode=='list'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','post.title_list.html') ?>
					
                
                <?php }elseif($__Context->textyle_mode=='summary'){ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','post.summary.html') ?>
                
                <?php }else{ ?>
					<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/textyle/skins/happyLetter','post.list.html') ?>
				<?php } ?>
        ...
        <hr />
		<div id="footer">
			<a href="<?php echo $__Context->home_url ?>/rss" class="rss"><img src="/modules/textyle/skins/happyLetter/img/iconRss.gif" width="16" height="16" alt="RSS" /></a>
			<div class="language">
